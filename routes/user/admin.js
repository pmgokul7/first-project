...
const orderManagement = require("../../helpers/admin/orderManagement")
        ...
        con.get().collection("orders").find({method: "paypal"}).toArray().then((paypal) => {
            ...
            con.get().collection("orders").find({method: "COD"}).toArray().then((cod) => {
                ...
                con.get().collection("orders").find({method: "razorpay"}).toArray().then((razor) => {
                    ...
                    con.get().collection("orders").aggregate([
                        {
                            $match: {
                                status: "placed",
                                method: "COD"
                        ...
                        con.get().collection("orders").aggregate([
                            ...
                            con.get().collection("orders").aggregate([
                                ...
                                res.render("admin/adminDash", {
                                    ...
                                    codR: codd[0].sumcod,
                                    paypalR: paypall[0].sumpaypal ? paypall[0].sumpaypal : 0,
                                    razorR: typeof(razorr[0].sumrazor) != 'undefined' ? razorr[0].sumrazor : 0,
                                    total: 5 + paypall[0].sumpaypal + codd[0].sumcod
                                });

                                console.log(typeof(sumrazor));
...
route.get("/products/edit", (req, res) => {
    con.get().collection("Products").findOne({
...
route.get("/orders", (req, res) => {
    ...
    orderManagement.getAllOrders(req).then(result => {
...
route.post("/orders/update", (req, res) => {
    ...
    con.get().collection("orders").updateOne({
        ...
        $set: {
            ...
            "product.$.deliverytime": req.body.statusDate
        }
    }).then(() => {
...
route.get("/categories", (req, res) => {
    ...
    con.get().collection("cat").find({}).toArray().then((result) => { // console.log(result);
        res.render("admin/categories", {result, msg});
    });
});

route.get("/deletecategory", (req, res) => {
    con.get().collection("cat").deleteOne({
        _id: ObjectId(req.query.id)
...
route.get("/editcat", (req, res) => {
    con.get().collection("cat").findOne({
        _id: ObjectId(req.query.id)
    }).then((result) => {
        var msg = req.flash('info')
...
route.get("/sub-categories", (req, res) => {
    con.get().collection("categories").find({}).toArray().then((categories) => { // console.log(categories[0].Brands);
        res.render("admin/sub-categories", {categories});
    });
});
route.post("/addBrand", (req, res) => {
    con.get().collection("categories").updateOne({}, {
        $push: {
            Brands: {
...
route.get("/reports", (req, res) => {
    ...
    con.get().collection("orders").find({
        date: {
            $lt: now,
            $gte: start
        }
        ...
        console.log(now);
        res.render("admin/reports", {result, heading: `Sales Report ${
                moment().format("YYYY")
            }`});
    });
});
...
route.get("/report/month", (req, res) => {
    ...
    const start = new Date(`${backYear},${backMonth},${
        now.getDate()
    }`);
    con.get().collection("orders").find({
        date: {
...
route.get("/returns", (req, res) => {
    con.get().collection("return").aggregate([
        {
            $match: {}
        },
        {
